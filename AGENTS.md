...
### 步骤
0. **巡检**：运行 `npm run check:freshness`，得到每个指标的最新采集日期与状态（新鲜 / 过半 / 过期 / 缺口）。存在「过期」时本轮必须重采对应来源，不得只留痕不改数据。
1. **采集**：从来源白名单获取数据（见下方清单）。
...
3. **构建校验**：运行 `npm run build`。scoring.ts 会自动执行：
   ...
   - 时效检查（过期 → 仅展示不入分）
   - 冲突检测（同日分歧 >10% → 降置信度）
4. **波及分析**：检查更新过的 subject 被哪些场景引用（`dimensions` 与 `product_id`），受影响场景页的排名会自动重算。
5. **留痕**：在 `changelog.yaml` 追加条目，写清采集了什么、校验结果、哪些排名受影响。
6. **复核**：再次运行 `npm run check:freshness`，确认全部有据指标回到「新鲜」；「缺口」（注册但无证据的指标）报给人工决定补采或降级。
...
| Artificial Analysis | tier 3 | 价格/速度交叉参考（仅参考，权重低） | https://artificialanalysis.ai |
...
- **巡检入口**：`npm run check:freshness`——按指标报告最新采集日期、距时效窗口的余量、待人工抽查条数；任何指标过期即非零退出，可作 CI 门禁。
- **同步范围**：来源白名单全部来源（tier 1 官方定价页逐家重采；arena.ai / swebench.com 榜单对照上次快照，有变化才写入）；`models.yaml` / `products.yaml` 的价格与版本摘要随证据同步，主观字段（可用性、订阅价）标待核实。
