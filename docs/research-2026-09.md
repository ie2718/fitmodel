...
- **综合榜结构性偏差修复（2026-09-12，用户反馈「榜单不客观，AA 前二排在靠后位置」）**：引擎级诊断（指标层拆解）定位三个病灶——① **缺席无代价**：opus-4-6-high 仅凭 4 条 arena 类目名次（难题/指令 #1、总榜 #2）拿通用 95.9 与 cov=1.00 高置信，而它缺席 AA v4.3 现役榜这一事实未被引擎消费；② **单类目劫持维度**：fable-5.1-max 编码维度被 arena 编码 #34（百分位 33.3）拖到 64.8，同实体其余 8 条实测在 80–96 区间（elo 89.4 / WebDev 96.1 / AA 88.8）；③ **同源信号叠乘**：4 个高相关 arena 类目名次在 coding/writing/longctx 三维度各拿 96–98，等价于同一投票人群的重复采样按独立证据计权。算法响应（全部公示于方法论第四步）：**缺席先验**（metrics.yaml `absence_prior`，AA = 30±24：现役精选榜下架被替代代际——GPT-5.6 先例——缺席即非现役证据，只对「维度内有其他实测」的实体注入，避免与第二段知情先验双重收缩）；**类目覆盖滞后折减**（名次低于自身其余实测加权中位数 40 分以上且其余 ≥4 条 → 可靠度 ×0.5，单向向下：覆盖滞后只会表现为名次异常低，与 Text 总榜曾缺 GPT-6 同理）；**纯名次维度收缩**（非现役实体的全名次维度与先验 50/50 收缩，含 SWE/AA/规格等区间型实测的维度豁免——真实任务与规格证据优先于偏好类目）。**效果**（CANONICAL 权重下综合榜）：fable-5.1-max #7→#2（84.7）、gpt-6-astra-max #6→#3（83.9），fable-5（arena+AA 双料第 1）居 #1（92.4）；opus-4-6-high 96.2→67.5、opus-4-7-high 92.9→65.5（其 arena 类目名次仍在「依据」列完整展示，收缩原因可追溯）。**残余妥协（诚实记录）**：缺席先验 30±24 与折减阈值 40/×0.5 是人为定标——比连续建模（如代际插值）粗糙，但可公示、可回查、不虚构任何数据；类目名次本身不删除，仪器分歧仍并列展示。若后续 SWE-bench 官方运行覆盖当前旗舰，应以真实任务数据复核本轮收缩是否过强。
- **kimi-k3-max vs gemini-3.8-flash 排序复核（2026-09-12，用户反馈「kimi 比 Gemini 3.8 Flash 低也不客观」）**：指标层拆解显示通用维度两者基本打平（68.3 vs 69.9），综合 8.6 分差距中 8.2 分来自写作维度单事实（kimi 创意写作 #25=百分位 51 vs gemini #4=92.2）。顺带发现一个真实仪器缺陷：**arena Elo 的 z-score 在拥挤前沿被噪声放大**——组内 σ 仅 8.07（n=16，极差 26 Elo），小于 LMArena 单模型 CI（±10–30），同一信号两种编码读数相差 50 百分位（kimi Elo=27 vs 总榜名次#12=76.5），证明分差主要是噪声。修复三项：① **测量噪声膨胀**（metrics.yaml `noise_sd`，Elo=±15，σ_eff=√(σ²+noise²)，噪声量级内的分差不再放大）；② **arena 通用家族去相关**（Elo/总榜名次/难题/指令各 weight 0.5，家族预算 2.0=AA 权重，通用维度偏好:能力=50:50）；③ **综合指数维度权重与证据厚度相称**（0.40/0.30/0.20/0.10 → **0.45/0.35/0.10/0.10**：写作是唯一无能力锚证据、仅单一偏好类目的维度；通用含 AA 锚 5+ 信号；编码三仪器）。**效果**：kimi-k3-max #17→#9（76.0），通用维度反超 gemini（69.8 vs 67.4），两者 CI 重叠（[65,87] vs [68,89]，统计并列）；gpt-6-astra-max 升至 #2（AA 锚直接受益）、fable-5 仍 #1、opus-4-6-high #34。**残余分歧（如实呈现，不掩饰）**：kimi 综合仍低于 gemini 2.6 分——唯一剩余来源是创意写作类目（#25 vs #4，英文盲测真实偏好读数，kimi 长板在中文写作而 arena 以英文为主）。若要抹平此差距只能把写作权重降为零或引入中文写作基准（未入白名单），前者使综合指数失去写作信息、后者待评估——维持现状并公开该分歧是当前最诚实的解。
- **社区口碑信号接入（2026-09-12，用户指令「更大范围搜索用户真实反馈，结合高质量榜单做排名」）**：新白名单来源（Reddit / HN / 知乎 / V2EX / linux.do）+ 新指标 `community_signal`（tier 3 · weight 1 · noise_sd 1.5 · 时效 30 天，约 6% 综合话语权）。**转译规则（预置公示后再采集）**：净信号 = 正/负独立讨论串数之差；只计针对能力与使用体验的具体可归因陈述；混合串不计；定价计入、自托管硬件门槛不计；媒体评测不算用户反馈；每模型 ≥2 独立串才入册；无数据变体不参与（不罚分）。**首轮采集 10 个变体**（Fable 5.1、GPT-6 Astra、Kimi K3、GLM-5.3、Gemini 3.8 Flash、DeepSeek V4 Pro/Flash、Qwen3.8 Max/27B/Flash-Next），逐条带原帖链接与正负计数。**关键发现**：① kimi-k3 口碑显著正面（开放权重庆祝、接近 Fable 水平、登 Code Arena 榜首）而 gemini-3.8-flash 正负相抵（downgrade/审查抱怨）——口碑信号使 kimi #9→#7 反超 gemini #8（区间重叠显示 ≈）；② qwen3.8-max 口碑负面（小改动大重写、Claude Code 风格过拟合）与其高分形成背离，降至 #41；③ 修复一个交互缺陷——口碑事实曾误触发 AA 缺席先验（口碑非能力实测，不应作为「维度内有实测」的判定依据，否则补口碑反而掉分），修复后 flash-next 等纯口碑实体由口碑分直接构成通用维度。**已知偏差**：自选样、英文社区为主、无法盲测、发布会周期情绪放大——因此仅给修正信号权重，全部引用可回查。**待办线索**：r/DeepSeek 出现「V4.1」讨论（疑似新版本，未入册）；GLM-5.3-Flash「agentic 长尾不可靠」的混合口碑未达入册门槛，下轮复核。
