...
export const GET: APIRoute = async ({ site, props }) => {
  const body = {
    ...metaBlock(site!, {
      description:
        'picks.top = 数据上最强；picks.value = 性价比最高（与 top 不同时给出）；picks.free = 有免费额度的国内可用产品中底座模型最优者。ranking 按适配分排序，ci90 区间重叠时 tied_with_previous=true（视为并列）。引用请注明 data_as_of 与来源链接。',
