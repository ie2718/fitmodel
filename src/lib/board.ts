...
/** 综合能力指数的维度权重（公示）。
 *  2026-09-12 调整：权重与证据厚度相称——通用 0.45（5+ 信号含 AA 能力锚）、编码 0.35（类目+WebDev+SWE 三仪器）、
 *  写作 0.1（唯一无能力锚证据、仅单一偏好类目的维度）、长上下文 0.1（名次+规格）。改动必须在 changelog 留痕。 */
...
export const DIM_DESC: Record<string, string> = {
  ...
  writing: 'Arena 创意写作类目盲测排名——写作维度唯一证据（无 AA/SWE 等能力锚覆盖），单仪器读数，综合指数权重据此降为 0.1；写作类场景页仍以本维度为主（场景独立权重）。',
